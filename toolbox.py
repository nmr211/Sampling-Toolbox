...
def load_data():
    GPI_file = 'geo500101g.002.hdf5'
    IMF_file = 'imf631127g.002.hdf5'
    AL_file = 'aei780101g.001.hdf5'
    WIND_file = 'Flattened_UVT_data_WACCM-X.nc'

    with h5py.File(GPI_file, 'r') as f:
        f107 = f['Data']['Table Layout']['f10.7'][:] / 1e-22
        f107 = f107[np.isfinite(f107)]

    with h5py.File(IMF_file, 'r') as f:
        bx = f['Data']['Table Layout']['bxgsm'][:] * 1e9
        by = f['Data']['Table Layout']['bygsm'][:] * 1e9
        bz = f['Data']['Table Layout']['bzgsm'][:] * 1e9
        swden = f['Data']['Table Layout']['swden'][:]
        swvel = f['Data']['Table Layout']['swspd'][:]

    with h5py.File(AL_file, 'r') as f:
        al = f['Data']['Table Layout']['al'][:]

    with nc.Dataset(WIND_file, 'r') as f:
        un = f.variables['un'][:21000]
